#!/usr/bin/env python3
"""Inspect or apply the approved Malaysia yearly subscription price change."""
...
def target_point(client: ASCClient) -> dict[str, Any]:
    matches = [
        point
        for point in price_points(client)
        if Decimal(point["attributes"]["customerPrice"]) == TARGET_CUSTOMER_PRICE
    ]
    if not matches:
        raise RuntimeError(f"No exact Apple price point for MYR {TARGET_CUSTOMER_PRICE}")
    return matches[0]
...
def main() -> None:
    ...
    if not any(
        ...
        raise RuntimeError("Apple Malaysia target price was not visible after the update")
