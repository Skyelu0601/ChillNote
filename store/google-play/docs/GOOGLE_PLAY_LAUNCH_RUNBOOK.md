...
- `store/google-play/assets/app-icon-512.png`
- `store/google-play/assets/feature-graphic-1024x500.png`
- `store/google-play/screenshots/en-US/`

## 第 2 步：创建并保管上传密钥（需要你操作）

原因：私钥和密码必须只由应用所有者掌握，不能交给 AI，也不能提交到 Git。

1. 按 `store/google-play/docs/GOOGLE_PLAY_SIGNING.md` 创建 `android/upload-key.jks`。
...
服务账号权限配置完成后，也可以在 `store/google-play/` 运行 `bundle exec fastlane android upload_internal`；该 lane 只上传到 Internal testing，并默认保持 Draft。Production 自动上传工具也已提供，但必须等本清单的正式发布门禁全部通过后再使用。
...
Play Console 与后端的具体配置见 `store/google-play/docs/GOOGLE_PLAY_BILLING_SETUP.md`。

1. 在 Play Console 创建：
   - `com.chillnote.pro.weekly`
   - `com.chillnote.pro.yearly`
2. 配置基础方案、地区和价格；周付与年付均不配置试用。
...
填写时以 `store/google-play/docs/GOOGLE_PLAY_DATA_SAFETY_DRAFT.md` 和
`store/google-play/docs/GOOGLE_PLAY_STORE_LISTING.md` 为草稿，但必须根据生产环境真实情况最后确认。
...
门禁全部通过后，使用两步式 Production 发布：

```text
# 上传 Production 草稿，不提交审核
bundle exec fastlane android build_and_upload_production version_code:7 confirm:PRODUCTION-7

# 在 Play Console 核对草稿后，提交完整发布和审核
bundle exec fastlane android submit_production version_code:7 confirm:RELEASE-7
```

把示例中的 `7` 换成 `android/app/build.gradle` 当前的 `versionCode`。完整参数和安全约束见 `store/README.md`。
